...
function randomelement(randomNum) {
    console.log(randomNum);
   
}

let randomNum = random30();
randomelement(randomNum);

let randomuser = users.find(user => user.id === randomNum);
 localStorage.setItem("users", JSON.stringify(randomuser));
...
function calculate(birthDate) {
    ...
    let age = date.getFullYear() - userdate.getFullYear(); 

    return age;
}

users.forEach(user => {
  let result =  user.age = calculate(user.birthDate); 
  console.log(result); 
});
